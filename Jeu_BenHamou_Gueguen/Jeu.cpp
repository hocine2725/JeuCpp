...
bool Jeu::loadMedia(Tile *tiles[], SDL_Renderer *gRenderer, SDL_Rect *gTileClips)
{
	...
	//Load dot texture
	if (!gPersonnageTexture.loadFromFile("Images/hero3.png", gRenderer))
	{
		printf("Failed to load hero texture!\n");
		success = false;
	}

	if (!gGardeTexture.loadFromFile("Images/policeman.png", gRenderer))
	{
		printf("Failed to load guard texture!\n");
		success = false;
	}

	if (!ArmeTexture.loadFromFile("Images/knife.png", gRenderer))
	{
		printf("Failed to load knife texture!\n");
		success = false;
	}

	if (!MoneyTexture.loadFromFile("Images/money.png", gRenderer))
	{
		printf("Failed to load money texture!\n");
		success = false;
	}

	if (!CleTexture.loadFromFile("Images/key.png", gRenderer))
	{
	...
	//Load tile texture
	if (!gTileTexture.loadFromFile("Images/carte.png", gRenderer))
	{
...
bool Jeu::accueil()
{
	...
	img = IMG_LoadTexture(gRenderer, "Images/menu.png");
	if (!img)
	{
		return false;
	}
...
bool Jeu::perdu()
{
	...
	img = IMG_LoadTexture(gRenderer, "Images/Perdu.png");
	if (!img)
	{
		return false;
	}
...
bool Jeu::gagne()
{
	...
	img = IMG_LoadTexture(gRenderer, "Images/Gagne.png");
	if (!img)
	{
		return false;
	}
